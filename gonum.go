...
var (
	...
	output    = flag.String("output", "", "output file name; default srcdir/enum.go")
)
					...
					if field.Tag != nil && strings.HasPrefix(field.Tag.Value, "`enum:") {
						...
						if len(field.Names) > 0 {
							e.elements = append(e.elements, enumElement{
								...
								name:  parseEnumStructTag(field.Tag.Value).name,
							})
						}
					}
				}
...
type enum struct {
	...
	newName      string // The name with trimmed prefix.
	elements     []enumElement
}
...
func New{{.NewType}}(value string) {{.NewType}} {
	switch value {
{{- range $e := .Fields}}
	case "{{.Key}}":
		return {{.Value}}
